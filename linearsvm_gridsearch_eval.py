...
def evaluate_svm(C, train_embeddings, train_labels, val_embeddings, val_labels, test_embeddings, test_labels, dataset):
    start_time = time.time()
    print(f"{datetime.datetime.now()} - Starting evaluation: dataset={dataset}, C={C}", flush=True)
    ...
    print(f"{datetime.datetime.now()} - Finished: dataset={dataset}, C={C} "
          f"in {elapsed_time:.2f} seconds. Test F1: {test_f1:.4f}", flush=True)

    model_filename = f"linearsvm_C{C}.joblib"
...
def append_results_to_csv(results, csv_path, lock):
    with lock:  # Ensure thread-safe writing
        pd.DataFrame([results]).to_csv(csv_path, mode='a',
                                       header=not os.path.exists(csv_path), index=False)


if __name__ == "__main__":
    embeddings_to_evaluate = ['r18dml431', 'r50dml431', 'har431', 'r101dml431']
    savepath = "linearsvm_gridsearch_results-pl.csv"
    ...
    for dataset in embeddings_to_evaluate:
        ...
        def load_embeddings(folder):
            ...
            for emb in os.listdir(os.path.join(embeddings_path, folder, "forest")):
                embeddings.append(np.load(os.path.join(
                    embeddings_path, folder, "forest", emb)).flatten())
                labels.append(0)
            for emb in os.listdir(os.path.join(embeddings_path, folder, "recent_def")):
                embeddings.append(np.load(os.path.join(
                    embeddings_path, folder, "recent_def", emb)).flatten())
                labels.append(1)
        ...
        if not os.path.exists(savepath):
            pd.DataFrame(columns=[
                'dataset', 'C', 'train_f1', 'train_acc', 'train_bal_acc', 'train_precision', 'train_recall', 'train_tnr', 
        ...
        with parallel_backend('loky', n_jobs=36):
            jobs = Parallel(verbose=10)(
                delayed(lambda c: append_results_to_csv(
                    evaluate_svm(c, train_embeddings, train_labels, val_embeddings,
                                 val_labels, test_embeddings, test_labels, dataset),
                    savepath,
                    lock
                ))(C)
