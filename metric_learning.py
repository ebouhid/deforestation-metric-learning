    ...
    for backbone_name in ["resnet18", "resnet50", "resnet101"]:
        ...
        for i in range(1, 4):
            ...
            train_dataset = SegmentClassificationDataset(
                root_dir=root_dir, regions=train_regions, transform=transform, return_dict=False)
            val_dataset = SegmentClassificationDataset(
                root_dir=root_dir, regions=val_regions, transform=False, return_dict=False)
            ...
            for epoch in range(1, num_epochs + 1):
                ...
                if val_loss < best_val_loss:
                    ...
                    torch.save(model.state_dict(), os.path.join("dml_results", backbone_name, f"best_model_state_dict_{i}.pth"))
            ...
            plt.savefig(os.path.join("dml_results", backbone_name, f"training_loss{i}.png"))

            plt.clf()
            plt.figure(figsize=(12, 6))
            plt.plot(val_info["epoch"], val_info["val_loss"], label="Validation Loss")
            plt.savefig(os.path.join("dml_results", backbone_name, f"validation_loss{i}.png"))

            # Save to .txt
            with open(os.path.join("dml_results", backbone_name, f"validation_loss{i}.txt"), "w") as f:
